...
class PlayingState: GKState {
    ...
    init(adapter: GameSceneAdapter) {
        ...
        guard let scene = adapter.scene else { return }
    ...
    override func didEnter(from previousState: GKState?) {
        ...
        if previousState is PausedState { return }
        
        guard let scene = adapter.scene, let player = adapter.playerCharacter else { return }
    ...
    private func position(player: PlayableCharacter, in scene: SKScene) {
        guard let playerNode = adapter.playerCharacter else { return }
