...
struct PipeFactory {
    ...
    static func launch(for scene: SKScene, targetNode: SKNode) -> SKAction {
        ...
        let producePipeAction = SKAction.run {
            ...
            guard var pipe = PipeFactory.producseDoublePipe(sceneSize: scene.size) else { return }
            if Bool.pseudoRandomPipe {
                guard let standardPipe = PipeFactory.produceStandardPipe(sceneSize: scene.size) else { return }
                pipe = standardPipe
            }
    ...
    private static func producseDoublePipe(sceneSize: CGSize) -> SKSpriteNode? {
        guard let pipeParts = PipeFactory.doublePipeParts(for: sceneSize) else { return nil }
    ...
    private static func standardPipeParts(for sceneSize: CGSize) -> PipeParts? {
        ...
        guard let unwrappedPipeTop = pipeTop else { return nil }
    ...
    private static func doublePipeParts(for sceneSize: CGSize) -> DoublePipeParts? {
        ...
        guard let unwrappedPipeMidUp = midUpPipe else { return nil }
        ...
        guard let unwrappedPipeMidDown = midDownPipe else { return nil }
